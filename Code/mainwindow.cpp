...
MainWindow::MainWindow(QWidget *parent) : QMainWindow(parent), brushActionGroup(this), doc(&undoStack)
{
    createMenus();
...
{
    bool ok;
    const int newWidth = QInputDialog::getInt(this, tr("Scribble"), tr("Select pen width:"), doc.getPenWidth(), 1, 50, 1, &ok);
    if (ok) {
...
void MainWindow::createMenus()
{
    ...
    for (const auto &actionDesc : shapeActions) {
        ...
        connect(act, &QAction::triggered, std::bind(&Document::setShapeFactory, &doc, actionDesc.second));
...
{
    if (doc.isModified()) {
       const QMessageBox::StandardButton ret = QMessageBox::warning(this, tr("Paint"), tr("The image has been modified.\n" "Do you want to save your changes?"), QMessageBox::Save | QMessageBox::Discard | QMessageBox::Cancel);
...
bool MainWindow::saveFile(const QByteArray &fileFormat)
{
    ...
    const QString fileName = QFileDialog::getSaveFileName(this, tr("Save As"), initialPath, tr("%1 Files (*.%2);;All Files (*)").arg(QString::fromLatin1(fileFormat.toUpper())).arg(QString::fromLatin1(fileFormat)));
    return !fileName.isEmpty() &&  doc.saveImage(fileName, fileFormat.constData());
}
