...
{
    return !undoStack->isClean();  //undo stack keeps the document's modification state
}
...
bool Document::openImage(const QString &fileName)
{
    ...
    image = loadedImage;
    ResizeCommand(this, &image, newSize).redo();  // reuse the command's functionality out of the undo stack
    undoStack->clear();
    return true;
}
...
void Document::paintEvent(QPaintEvent *event)
{
    ...
    painter.setRenderHint(QPainter::Antialiasing, true);    //This is to avoid some pesky artefacts on some platforms
    painter.drawImage(paintRect, image, paintRect);// just get the required bit of image updated on the widget
    if (currentShape) {    // ... and draw a shape if there's any under construction
