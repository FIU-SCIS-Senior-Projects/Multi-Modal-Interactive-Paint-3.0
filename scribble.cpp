...
Scribble::Scribble(const QPoint &topLeft, int penWidth, const QColor &penColor) : Shape(penWidth, penColor)
{
    update(topLeft);
}
...
{
    return std::unique_ptr<Shape>(new Scribble(topLeft, penWidth, Qt::white));
}
